...
---
### 强制规则
- **开发环境**：当前环境是 **Windows**，所有命令和路径必须使用 Windows 格式
  - 路径分隔符：使用 `\` 或在 Bash 工具中使用 `/`
  - 切换目录：使用绝对路径，例如 `cd C:\Users\...\erp_server`
  - 禁止使用相对路径 `cd erp_server`，必须使用完整路径
- **代码分析**：优先调用 LSP 能力来分析代码结构、符号和上下文，而不是只靠文本理解
- **代码处理**：每次修改，必须提交git
- **回滚机制**：当用户表示不满意或遇到报错无法修复时，直接执行 `git reset --hard HEAD~1` 回滚到上一个版本
